...
const LoteriasResumoPage = () => {
  ...
  const isValendoLocked = !!draft?.valendoLocked;

  const safeDigits = (s) => String(s || '').replace(/\D/g, '');

  const getValendoBase = (d) => {
    const apostas = Array.isArray(d?.apostas) ? d.apostas : [];
    if (!apostas.length) return { modalidadeBase: '', numerosBase: [] };
    const base = apostas[0];
    const numerosBase = Array.isArray(base?.palpites) ? base.palpites : [];
    const modalidadeBase = base?.modalidade || d?.modalidade || '';
    return { modalidadeBase, numerosBase };
  };

  const getBaseDigits = (numerosBase) => {
    const sizes = (numerosBase || []).map((p) => safeDigits(p).length).filter(Boolean);
  ...
  const startValendo = () => {
    const d = getDraft();
    const { modalidadeBase, numerosBase } = getValendoBase(d);
    const baseDigits = getBaseDigits(numerosBase);
    if (!baseDigits || !numerosBase.length) return;
  ...
  const handleRemoveAposta = (idx) => {
    ...
    if (!next.length) {
      updated.isValendo = false;
      updated.valendoLocked = false;
      updated.valendoBase = null;
    }
    updateDraft(updated);
    setDraft(updated);
  };
  ...
  const canShowValendoBtn = useMemo(() => {
    ...
    const { numerosBase } = getValendoBase(draft);
    const baseDigits = getBaseDigits(numerosBase);
    return hasApostas && (baseDigits === 3 || baseDigits === 4);
  }, [draft]);
